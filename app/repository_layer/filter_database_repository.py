...
from app.repository_layer.base_filtersets import BaseTasklyFiltersetRules
from app.repository_layer.models.models import DatabaseBaseModel, Filter


class FilterDatabaseRepository(AbstractDatabaseRepository):
    ...
    async def get_filterset(self, params: dict, session: AsyncSession) -> Any:
        ...
        e.g.
        base_query = select(Filter) # equivalent to select * from [filter table]
        filterset  = TaskFilterset(session=session,query=base_query)"""
        base_query = select(Filter)
        filterset = BaseTasklyFiltersetRules(session=session, query=base_query)
        return filterset

    @property
    async def model_class(self):
        """Return the database model class e.g. return Filter"""
        return Filter
