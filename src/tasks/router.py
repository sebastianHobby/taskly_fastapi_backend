...
@task_router.get("/tasks", status_code=status.HTTP_200_OK, response_model=list[schemas.TaskGet])
async def get_all_tasks(database: SessionDep):
    tasks_query = database.query(models.Task)
    return tasks_query.all()

@task_router.get("/tasks/{task_id}", status_code=status.HTTP_200_OK, response_model=schemas.TaskGet)
...
async def update_task(database: SessionDep, task_request: schemas.TaskUpdate, task_id: str):
    try:
        uuid = UUID(task_id)
    except ValueError:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail = "Task not found")
