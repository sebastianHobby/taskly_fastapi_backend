...
from src.schemas.TaskSchemas import TaskSelect, TaskCreate, TaskUpdate
...
class TasklyTaskService:
    ...
    async def _validate_update_or_create(self, data: Union[TaskUpdate, TaskCreate]):
        ...
        if await self.exists(name=data.name, parent_group_id=data.parent_list_id):
            raise TasklyDuplicateData(
                message=f"Task '{data.name}' already exists under group '{data.parent_list_id}'",
            )

    async def _validate_delete(self, _id):
        pass

    async def create(self, create_schema: CreateSchemaType, commit=True) -> TaskSelect:
        """
        ...
        async with self.database_session_factory() as session:
            ...
            res = await self.repository.create(
                ...
                commit=commit,
                schema_to_select=TaskSelect,
                return_as_model=True,
            )
            return res

    async def get(self, _id: UUID) -> TaskSelect:
        """
        Create a new record in the database.
        Args:
        ...
        async with self.database_session_factory() as session:
            ...
            if not res:
                raise TasklyServiceException(
                    ...
                    message=f"Resource for id {_id} not found",
                )
            return res
    ...
    async def update(
        ...
        Args:
            ...
            commit: If `True`, commits the transaction immediately. Default is `True`.
            _id: UUID of the record to update.
    ...
    ) -> UpsertMultiResponseModel[TaskSelect]:
        """
